...
if __name__ == '__main__':
    GT = pd.read_csv('F:\Projects\PycharmProjects\opencvtest\detection\data\TownCentre-groundtruth.top', header=None)
    ...
    for frame_number in range(train_size):
        ...
        with open(os.path.join('data\VOC2012\Annotations', str(frame_number) + '.xml'), 'w') as file:
            file.write('<annotation>\n')
            file.write(indent('<folder>voc2012</folder>\n', 1))
            file.write(indent('<filename>' + str(frame_number) + '.jpg' + '</filename>\n', 1))
            file.write(indent('<path>F:/Projects/PycharmProjects/opencvtest/detection/data/VOC2012/JPEGImages/' + str(frame_number) + '.jpg' + '</path>\n', 1))
